...
describe('Alias Redirect Page', () => {
    ...
    it('redirects to target URL for valid alias', async () => {
        ...
        const params = Promise.resolve({ identifier: 'my-alias' });
        
        await expect(Page({ params })).rejects.toThrow('REDIRECT:https://example.com/alias-target');
    ...
    it('returns 404 for non-existent alias', async () => {
        ...
        const params = Promise.resolve({ identifier: 'non-existent-alias' });
    ...
    it('returns 404 for disabled URL', async () => {
        // The query includes .eq('enabled', true), so disabled URLs return null
        mockMaybeSingleResult = { data: null, error: null };

        vi.resetModules();
        const { default: Page } = await import('./page');
        
        const params = Promise.resolve({ identifier: 'disabled-alias' });
